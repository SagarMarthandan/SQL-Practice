...
SELECT
	REPLACE (title, ' ', '-') AS modified_title
FROM
	books;
...
SELECT
	UPPER(CONCAT (author_fname, ' ', author_lname)) AS full_name
FROM
	books;

--WHERE author_lname LIKE 'G%';
SELECT
	CONCAT (title, ' was released in ', released_year) AS book_info
FROM
	books;
